...
/* GET THE NAMES IN SORTED ORDER */
   ...
   HeaderList = SPDXMalloc( sizeof(STFH)*ActualNamesFound );
...
/* OPEN/CREATE/TRUNC THE STF OUTPUT FILE */
   ...
   Handle = SPDXOpen( OutputName, O_BINARY|O_RDWR|O_CREAT|O_TRUNC, S_IREAD|S_IWRITE );
...
/* WRITE OUT THE HEADER FOR THE STF FILE */
   ...
   if (SPDXWrite(Handle, (CHR *) HeaderList, sizeof(STFH)*ActualNamesFound) != sizeof(STFH)*ActualNamesFound )
      ErrorExit( E_NOWRITE );
...
/* COPY THE FILES INTO THE STF FILE */
   ...
   {
      TempHandle = SPDXOpen(NameList[i], O_BINARY|O_RDONLY, 0 );
