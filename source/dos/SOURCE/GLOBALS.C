...
DBL   dTempX, dTempY, dx, dy, Intensity;
   ...
   SPDXGlobals.LightList = SPDXMalloc( MAX_LIGHTS * sizeof(LGT *) );
   ...
   SPDXGlobals.SineTable = SPDXMalloc( (SIN_TAB_LEN+1) * sizeof(FLT) );
   ...
   SPDXGlobals.CosTable = SPDXMalloc( (SIN_TAB_LEN+1) * sizeof(FLT) );
   ...
   SPDXGlobals.PhongTab = SPDXMalloc( PHONG_TAB_LEN * PHONG_TAB_LEN * sizeof(INT) );
