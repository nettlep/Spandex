...
MAT   MatA, MatB, MatX, MatY, MatZ;
   ...
   SPDXMakeXMatrix(MatX, Rotation->x);
   SPDXMakeYMatrix(MatY, Rotation->y);
   SPDXMakeZMatrix(MatZ, Rotation->z);
...
OBJ   *Object;
   ...
   {
      if (!strnicmp( Universe->Name, Name, Len ))
         return Universe;
   }
