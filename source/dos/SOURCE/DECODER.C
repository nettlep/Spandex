...
static   CHR   CurrentByte;                     // Current byte
static   CHR   CurrentBlock[257];               // Current block
static   CHR   *PointerToNextByteInBlock;       // Ptr to next byte in block
...
static CHR  stack[MAX_CODES + 1];               /* Stack for storing pixels   */
static CHR  suffix[MAX_CODES + 1];              /* Suffix table               */
...
INT   SPDXDecompress(INT Handle, INT LineWidth, CHR *Buffer, INT ImgWidth, ifunptr WriteLine)
{
register CHR   *sp, *bufptr;
...
INT            Offset = 0;
   ...
   if ((buf = (CHR *) SPDXMalloc(ImgWidth + 1)) == NULL)
      return LE_NOMEMIMG;
