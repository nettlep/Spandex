...
P2D   *PA, *PB, *PC, *pTemp;
   ...
   if (iTopHeight)
   {
      ...
      if (fShortDX < fLongDX)
         while( iTopHeight-- )
         {
            SPDXBlastBytes((CHR *) pScanStart+FXD_TO_INT(fShortX), iColor, ROUND_FXD_TO_INT(fLongX) - FXD_TO_INT(fShortX) );
            pScanStart += iPitch; fShortX += fShortDX; fLongX += fLongDX;
         }
      else
         while( iTopHeight-- )
         {
            SPDXBlastBytes((CHR *) pScanStart+FXD_TO_INT(fLongX), iColor, ROUND_FXD_TO_INT(fShortX) - FXD_TO_INT(fLongX) );
            pScanStart += iPitch; fShortX += fShortDX; fLongX += fLongDX;
         }
   }
