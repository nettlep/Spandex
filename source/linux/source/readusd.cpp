...
static   INT   StoreUSDLine( UCHR *Bytes, INT LineLength, UCHR *Buffer, INT Width, INT Y );
...
INT   TotalMem, ArrayLength, ArrayCount;
   ...
   FileData = (UCHR *) SPDXMalloc( ArrayLength * ArrayCount );
   ...
   ProcessFile( (CHR **) &FileData, Universe );
...
INT   StoreUSDLine( UCHR *Bytes, INT LineLength, UCHR *Buffer, INT Width, INT Y )
{
   FUNC("StoreUSDLine");
   Assert( Bytes );
   Assert( Buffer );
...
CHR   Word[80];
   ...
   *FileData = GetNextWord( *FileData, Word );
   ...
   {
      if (!strncasecmp( Word, ByteParseList[i].Label, strlen(ByteParseList[i].Label)))
      {
...
CHR   Word[80];
   ...
   *FileData = GetNextWord( *FileData, Word );
   ...
   for( i = 0; ObjectParseList[i].Handler; i++ )
      if ( !strncasecmp( Word, ObjectParseList[i].Label, strlen(ObjectParseList[i].Label) ) )
...
CHR   Word[80];
   ...
   *FileData = GetNextWord( *FileData, Word );
   ...
   for( i = 0; MaterialParseList[i].Handler; i++ )
      if ( !strncasecmp( Word, MaterialParseList[i].Label, strlen(MaterialParseList[i].Label) ) )
...
CHR   Word[80];
   ...
   *FileData = GetNextWord( *FileData, Word );
   ...
   for( i = 0; LightParseList[i].Handler; i++ )
      if ( !strncasecmp( Word, LightParseList[i].Label, strlen(LightParseList[i].Label) ) )
