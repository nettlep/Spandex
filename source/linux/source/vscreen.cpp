...
UCHR  *TempScreen;
   ...
   if (!SPDXGetVirtualBuffer(VScreen))
      TempScreen = (UCHR *) SPDXMalloc(ResX * ResY);
   else
      TempScreen = (UCHR *) SPDXRealloc(VScreen->DrawScreen, ResX * ResY);
#endif
...
void  SPDXGetVirtualCenter( INT *CenterX, INT *CenterY, VSCR *VScreen )
{
   ...
   *CenterX = (INT) VScreen->Center.x;
   *CenterY = (INT) VScreen->Center.y;
}
...
void  SPDXSetVirtualPixel( UINT X, UINT Y, CHR Color, VSCR *VScreen )
{
   ...
   if (X >= (UINT) VScreen->iResX || Y >= (UINT) VScreen->iResY)
      return;
...
{
INT   x, y, col, i, addr, height, width, len;
   ...
   for( i = 0; i < len; i++ )
   {
      if ((UCHR) String[i] < (UCHR) FONT_CHAR_OFFSET || (UCHR) String[i] > (UCHR) 127)
         continue;
...
INT   x, y, YOffset;
INT   ResX, ResY;
UCHR  *MixTab, *ImageTop, *ImageBot, CurrMix, NextMix, *Image;
