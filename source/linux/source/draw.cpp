...
UCHR   *Intensity;
   ...
   for( i = 0; i < Count; i++ )
   {
      ...
      if (Ver->ShadeModel & (GOURAUD | LAMBERT))
      {
         ...
         SPDXSetVirtualPixel( FLT_TO_INT(Ver->Scr.x), FLT_TO_INT(Ver->Scr.y), Intensity[FLT_TO_INT(Ver->Int) * 256], VScreen);
      }
      else
      {
         SPDXSetVirtualPixel( FLT_TO_INT(Ver->Scr.x), FLT_TO_INT(Ver->Scr.y), SPDXGlobals.PalTab[FLT_TO_INT(Ver->Material->Color) *
