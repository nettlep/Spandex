...
static   INT   StoreMapLine( UCHR *Pixels, INT LineLength, UCHR *Buffer, INT ImgWidth, INT Y );
...
INT   SPDXInitMaps( )
{
   ...
   SPDXGlobals.PalTab = (UCHR *) SPDXMalloc( MAP_COLORS * MAP_SHADES );
   ...
   SPDXGlobals.TransTab = (UCHR *) SPDXMalloc( MAP_COLORS * MAP_COLORS );
...
CHR   Name[DEF_NAME_LEN+1];
   ...
   TempPtr = (UCHR *) SPDXMalloc( MAP_COLORS * MAP_SHADES );
   ...
   for( i = 0; i < MapCount; i++)
   {
      ...
      pTemp = (UCHR *) SPDXMalloc( XRes * YRes );
...
INT   StoreMapLine( UCHR *Pixels, INT LineLength, UCHR *Buffer, INT ImgWidth, INT Y )
{
   FUNC("StoreMapLine");
   Assert( Pixels );
   Assert( Buffer );
...
INT   i;
   ...
   for( i = 0; i < SPDXGlobals.MapCount; i++)
   {
      if (!strcasecmp(SPDXGlobals.MapList[i].Name, Name))
