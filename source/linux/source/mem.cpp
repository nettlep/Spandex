      ...
      while( Cur )
      {
         if (SPDXGetDebugState() & DEBUG_MEM)
         {
            if (SPDXGetDebugState() & DEBUG_FUNCS)
               sprintf( Str, "Memory leak At:0x%08X Size:%d Owner:%s", (UINT) Cur->Address, Cur->Size, Cur->Owner );
            else
               sprintf( Str, "Memory leak At:0x%08X Size:%d", (UINT) Cur->Address, Cur->Size );
...
void  *Ptr;
   ...
   {
      if (SPDXGetDebugState() & DEBUG_FUNCS)
         sprintf( TempStr, "%s [%s|%d] called SPDXMalloc for %d bytes (at 0x%08X)", OwnerFunc, OwnerFile, OwnerLine, Size-WRAP_SIZE, (UINT) Ptr );
      else
         sprintf( TempStr, "SPDXMalloc for %d bytes (at 0x%08X)", Size-WRAP_SIZE, (UINT) Ptr );
...
MEMLIST  *Cur = MemArray;
   ...
   {
      if (SPDXGetDebugState() & DEBUG_FUNCS)
         sprintf( TempStr, "%s [%s|%d] called SPDXRealloc for %d bytes (at 0x%08X)", OwnerFunc, OwnerFile, OwnerLine, Size-WRAP_SIZE, (UINT) Address );
      else
         sprintf( TempStr, "SPDXRealloc for %d bytes (at 0x%08X)", Size-WRAP_SIZE, (UINT) Address );
...
MEMLIST  *Cur = MemArray, *Prev = 0;
   ...
   {
      if (SPDXGetDebugState() & DEBUG_FUNCS)
         sprintf( TempStr, "%s [%s|%d] called SPDXFree of memory at 0x%08X", OwnerFunc, OwnerFile, OwnerLine, (UINT) Address );
      else
         sprintf( TempStr, "SPDXFree of memory at 0x%08X", (UINT) Address );
   ...
   while( Cur->Address != Address )
   {
      ...
      {   
         if (SPDXGetDebugState() & DEBUG_FUNCS)
            sprintf( TempStr, "%s [%s|%d] Unable to find free address in mem list at 0x%08X", OwnerFunc, OwnerFile, OwnerLine, (UINT) Address );
         else
            sprintf( TempStr, "Unable to find free address in mem list at 0x%08X", (UINT) Address );
...
MEMLIST  *Cur;
   ...
   {
      if (Ptr[i] != (CHR) HDR_BYTES)
      {
         if (SPDXGetDebugState() & DEBUG_FUNCS)
            sprintf( TempStr, "Memory Overrun (< 0) at address 0x%08X, Owner is %s", (UINT) Ptr, Cur->Owner );
         else
            sprintf( TempStr, "Memory Overrun (< 0) at address 0x%08X", (UINT) Ptr );

         SPDXLogError( LE_NOMEM, TempStr );
         return 1;
      }

      if (Ptr[i+Cur->Size+HDR_SIZE] != (CHR) HDR_BYTES)
      {
         if (SPDXGetDebugState() & DEBUG_FUNCS)
            sprintf( TempStr, "Memory Overrun (> Size) at address 0x%08X, Owner is %s", (UINT) Ptr, Cur->Owner );
         else
            sprintf( TempStr, "Memory Overrun (> Size) at address 0x%08X", (UINT) Ptr );
