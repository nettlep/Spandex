...
/* CREATE THE PALETTE LISTS */
   ...
   printf( "Loading    [%*s]", (int) ActualNamesFound, " " );
   printf( "\rLoading    [" );
...
#ifdef USE_NEUQUANT
   SPDXNeuquantImages( ImageList, ActualNamesFound, ActualNamesFound, 8, MapPalette );
#else
   SPDXOctreeQuantizeImages( ImageList, ActualNamesFound, ActualNamesFound, MAP_COLORS, 8, MapPalette );
#endif
...
/* CREATE THE TRANSPARENCY MAP & REDUCE COLORS */
   ...
   if (MapFlags & MAP_FLAGS_TRANS)
   {
      ...
      TempPtr = (CHR *) ImageList2[1]->Buffer;
...
#ifdef USE_NEUQUANT
   SPDXNeuquantImage( TempImage, 8, MapPalette );
#else
   SPDXOctreeQuantizeImage( TempImage, MAP_COLORS, 8, MapPalette );
#endif
...
#ifdef USE_NEUQUANT
      _SPDXNeuquantMakeTransTable( ImageList2[0], ImageList2[1], 8, MapPalette, Opacity );
#else
      _SPDXOctreeMakeTransTable( ImageList2[0], ImageList2[1], MAP_COLORS, 8, MapPalette, Opacity );
#endif
   }
   else
   {
#ifdef USE_NEUQUANT
      SPDXNeuquantImage( ImageList2[0], 8, MapPalette );
#else
      SPDXOctreeQuantizeImage( ImageList2[0], MAP_COLORS, 8, MapPalette );
#endif
   }
...
/* OPEN/CREATE/TRUNC THE MAP OUTPUT FILE */
   ...
   MapHandle = SPDXOpen( OutputName, O_RDWR|O_CREAT|O_TRUNC, S_IREAD|S_IWRITE );
...
/* ADD ALL THE FRAMES TO THE MAP FILE */
   ...
   printf( "Writing    [%*s]", (int) ActualNamesFound, " " );
   printf( "\rWriting    [" );
