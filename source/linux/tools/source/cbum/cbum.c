...
char _FUNCTION_[] = "cbum";
   ...
   for( i = 0; i < MapCount; i++)
   {
      ...
      SPDXGlobals.MapList[i].Data = (UBYT *) SPDXMalloc( SPDXGlobals.MapList[i].XRes*SPDXGlobals.MapList[i].YRes );
...
INT   ret;
   ...
   if (ret != Count)
   {
      printf( "\n\nERROR --> Unable to read from file (%ld/%ld)\n",
	      ret, Count );
...
INT   ret;
   ...
   if (ret != Count)
   {
      printf( "\n\nERROR --> Unable to write to file (%ld/%ld)\n",
	      ret, Count );
      printf( "            error[%d]:  %s\n", errno, sys_errlist[errno] );
      exit(1);
   }
